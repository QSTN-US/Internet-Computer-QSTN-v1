  ...
  return (
    <Modal show={isOpen} size="sn" width="100%" height="100%">
      <div className="mx-auto flex h-full w-full flex-col items-center justify-center space-y-4">
        <img src="/assets/images/logo.png" className="w-2/3" />
        ...
        {Object.keys(wallets).map((w) => (
          ...
          >
            <img src={'/assets/images/' + w + '.png'} className="h-8" />
            <span className="px-2 font-medium text-white">
              {'Connect ' + w[0].toUpperCase() + w.slice(1) + ' Wallet'}
            </span>
          </button>
        ))}
        <button onClick={() => isOpen('error')} className="hover:opacity-80">
          Close
        </button>
      </div>
    </Modal>
...
export const selectWalletModalPromise = createModal(SelectWalletModal);
