...
actor class EscrowCanister(
    ...
    let CROWDFUNDNFT_ACCOUNT_ICP = "c564482ad5865db064ace3f232efe0f17af5a4b0dff7f9947c176908f5448bcc";
    ...
    type SurveyId = Types.SurveyId; // Nat
    ...
    public func get_balance() : async Nat64 {
        let accountId : AccountIdText = await defaultAccountId("ICP");
        await accountBalance(accountId, "ICP");
    };
    ...
    let Backend = actor(backendPrincipal) : actor {
        getSurveyState : shared SurveyIdText -> async SurveyState;
    };
    ...
    stable var surveyState : SurveyState = #closed;
    public query func getSurveyState() : async SurveyState {
        surveyState;
    };
    public func updateSurveyState () : async SurveyState {
        surveyState := await Backend.getSurveyState(Nat.toText(surveyId));
        surveyState;
    };
    let CNFT_NFT_Canister = actor "aovwi-4maaa-aaaaa-qaagq-cai" : actor {
    ...
    public func getNewAccountId (principal: Principal, tier: NFTInfoIndex, network: SubaccountNetwork, refundWalletAddress: Text, email: Text) : async Result.Result<AccountIdText, Text> {
        if (getNumberOfUncancelledSubaccounts(tier) >= nfts[tier].number + oversellNFTNumber(nfts[tier].number)) return #err("This survey or survey tier is fully funded (or almost there, so we are pausing new transfers for the time being).");
        //if (endTime * 1_000_000 < Time.now()) return #err("Survey is past crowdfund close date.");
        if (maxNFTsPerWallet > 0 and principalNumSubaccounts(principal) >= maxNFTsPerWallet) return #err("This survey only allows each wallet to back the survey " # Nat.toText(maxNFTsPerWallet) # " times. You have already attained this maximum.");
        ...
        switch (surveyState) {
            case (#whitelist(whitelist)) {
                ...
                return #err("Survey is not open to funding.");
            }; case _ {};
        };
    ...
    public func returnFunds () : async () {
        assert(endTime * 1_000_000 < Time.now() and surveyIsFullyFunded() == false);
            ...
            case (?pssi) {
                if (pssi.1 == #empty) {
                    ...
                    if (pssi.1 == #confirmed) {
                        ...
                        return #err("You took longer than " # Nat.toText(emptyAccountCutOff) # " minutes to transfer the funds. In order to make sure that crowdfunding surveys don't get 'frozen' by people who click the crowdfund button but don't transfer quickly, we don't accept transfers past this cutoff. You will be refunded within the next few minutes, and if the survey is not yet fully funded, you can attempt to fund it again. Thank you for your understanding.");
                    };
                };
            };
